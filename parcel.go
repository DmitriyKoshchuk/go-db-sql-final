...
func (s ParcelStore) SetAddress(number int, address string) error {
	...
	if rowsAffected == 0 {
		return fmt.Errorf("нельзя изменить адрес для посылки со статусом %q", ParcelStatusRegistered)
	}
...
func (s ParcelStore) Delete(number int) error {
	...
	if rowsAffected == 0 {
		return fmt.Errorf("нельзя удалить посылку со статусом %q", ParcelStatusRegistered)
	}
